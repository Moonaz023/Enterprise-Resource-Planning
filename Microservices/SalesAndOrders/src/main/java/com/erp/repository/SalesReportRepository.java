...
public interface SalesReportRepository extends JpaRepository<SalesReportEntity, Long> {
	...
	@Query("SELECT s FROM SalesReportEntity s WHERE s.distributor_id=:distributorId")
	List<SalesReportEntity> findByDistributor_id(@Param("distributorId") long distributorId);
